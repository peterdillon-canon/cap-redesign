...
$(document).ready(function() {
  ...
  /* Related / Learning Content && Recommended Accessories Carousel */
    ...
    $(document).on('click', '.checkAgree', function () {        
		if ( $(this).closest('.row').find('.cbtn').hasClass('cbtn-canon-red')){
		    $(this).closest('.row').find('.cbtn').popover('disable');
        } else {
             $(this).closest('.row').find('.cbtn').popover('enable');
        }        
    });
	...
	*/
    ...
    function hasScrolled() {
        ...
        if (st > lastScrollTop){
            // Scroll Down            
            $('.navbar-left').removeClass('active-child');
            $('.nav-ribbon, .breadcrumbs, .wpToolbarActionBar').slideUp(300);
            $('#siteControl').css({ 'border-bottom': "3px solid grey"});
            var w = $(document).width();			
			if (w >= 768) {
            ...
            if(            		
                ...
                $('.nav-ribbon, .breadcrumbs, .wpToolbarActionBar').slideDown(300);
                $('#siteControl').css({ 'border-bottom': "0px"});
            }            
        }
...
// YouTube API is blocked by Canon.
...
function playVideo(source){
    ...
    $(btn).click(function() {
	    var h = $(this).height();
        $($(this) .find( ' ~ .video-container') ).height(h);
        $(this).hide();
...
var OsAutoDetector = {
		detect: function() {
			...
			if (osType == "LINUX"){
				if(osVersion != null && userAgent.indexOf("_64") != -1) {
					osVersion += "_64BIT";
				}else if(osVersion != null && userAgent.indexOf("_32") != -1) {
					osVersion += "_32BIT";
				}
			}
	...
	OsAutoDetector.OS_VER_REGEX_LIST = [
		...
		,{type:"MAC", version:"MACOS_10_14", regex:/Mac OS X 10(\.|_)14(;|_)?/}
		,{type:"MAC", version:"MACOS_10_13", regex:/Mac OS X 10(\.|_)13(;|_)?/}
		,{type:"MAC", version:"MACOS_V10_12", regex:/Mac OS X 10(\.|_)12(;|_)?/}
	...
	function checkMobileVersion() {
		...
		var platform = navigator.platform;
		var userAgent = navigator.userAgent;
		var mVersion = parseInt(appVersion,10);
		
		if (appVersion.indexOf("Win")!=-1) system="Windows";
		if (appVersion.indexOf("Mac")!=-1) system="MacOS";
		if (appVersion.indexOf("X11")!=-1) system="UNIX";
		if (appVersion.indexOf("Linux")!=-1) system="Linux";

		if (appVersion.indexOf("android")!=-1) system="Android";
		if (appVersion.indexOf("windows phone")!=-1) system="Windows Phone";
		...
		if (system.toLowerCase() == 'android'){
		...
		else if (system.toLowerCase() == 'windows phone'){
			...
			platform.toLowerCase() == 'ipod' ||
			platform.toLowerCase() == 'ipad' ||
			platform.toLowerCase() == 'iphone simulator'
			){
			var uaindex = userAgent.indexOf('OS ');
			var osVersion = userAgent.substr(uaindex + 3, 3 ).replace( '_', '.' );
			if (Number(osVersion.substr(0,2)) <=7){
				return true;
			}	
		}
		return false;	
